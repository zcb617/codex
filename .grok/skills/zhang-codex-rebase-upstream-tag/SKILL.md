...
| Final fork CLI publisher (`build-windows-codex.yml` on `rust-v*.*.*`) | Stacked tiny workflow edits |
...
`on.push.tags` must be `rust-v*.*.*`. The `publish-github-release` job must still exist.
...
rg -n 'rust-v\*\.\*\.\*|publish-github-release' .github/workflows/build-windows-codex.yml
```
...
- [ ] `publish-github-release` exists and runs only when `github.ref_type == 'tag'`
